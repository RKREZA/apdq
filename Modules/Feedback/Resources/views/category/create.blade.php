...
@section('page_title')
{{ __('core::core.create.title', ['name' => __('feedback::feedback.category.name')]) }}
@endsection
...
@section('container')
    ...
    <form id="feedback_category_add_form" action="{{ route('admin.feedbackcategories.store') }}" method="POST" role="form" autocomplete="off">
        @csrf()
...
<script>
// Validation
var validation_id               = "#feedback_category_add_form";
        var errorElement                = "em";
        var rules                       = {
            name: {
                required: true,
